...
class UserController {
            ...
            if (!userExsist) {
                ...
                if (userExsist.isActive == false) {
                    ...
                    if (password === userExsist.password) {
                        const token = jsonwebtoken_1.default.sign({ nickname: userExsist.nickname, phone: userExsist.phone }, process.env.SECRET_KEY, { expiresIn: "7d" });
                        let userIpAddress = req.headers["x-forwarded-for"]?.split(",")[0] ||
    ...
    async registerViaOtp(req, res) {
        try {
            ...
            const token = jsonwebtoken_1.default.sign({ nickname, phone }, process.env.SECRET_KEY, { expiresIn: "7d" });
            // Avoid P2002 on repeated attempts: upsert by phone
            const userPending = await database_1.default.userPending.upsert({
                where: { phone },
    ...
    async verifyOtp(req, res) {
        try {
            ...
            // }
            const userPending = await database_1.default.userPending.findUnique({ where: { phone: res.locals.payload.phone, status: true } });
            if (!userPending) {
                res.status(403).json({
                    message: "Kod eskirgan"
                });
                ...
                await database_1.default.userPending.update({ where: { phone: userPending.phone }, data: { status: false } });
                res.status(200).json({
                    message: "Ro'yhatdan o'tish muvafaqqiyatli yakunlandi!",
                    user: userWithToken
                });
    ...
    async getAll(req, res) {
        try {
            const { page, limit, search, isActive, isLoggedIn, rolesAny, rolesAll, createdFrom, createdTo, salaryMin, salaryMax, companyId, orderBy, sort, } = req.body;
            // helperlar
            const toBool = (v) => typeof v === "string" ? (v.toLowerCase() === "true" ? true : v.toLowerCase() === "false" ? false : undefined) : undefined;
            const parseRoles = (v) => {
                if (!v)
                    return undefined;
                if (Array.isArray(v))
    ...
    async findByPhone(req, res) {
        try {
            ...
            if (!phone)
                return res.status(400).json({ message: "phone talab qilinadi" });
    ...
    async findByNickname(req, res) {
        try {
            ...
            if (!nickname)
                return res.status(400).json({ message: "nickname talab qilinadi" });
    ...
    async checkNicknameCreated(req, res) {
        try {
            const { nickname } = req.body;
            if (!nickname)
                return res.status(400).json({ message: "nickname talab qilinadi" });
            const user = await userService.findByNickname(nickname);
            if (!user)
                return res.status(404).json({ message: "nickname ochiq" });
    ...
    async checkPhoneCreated(req, res) {
        try {
            const { phone } = req.body;
            if (!phone)
                return res.status(400).json({ message: "phone talab qilinadi" });
            const user = await userService.findByPhone(phone);
            if (!user)
                return res.status(404).json({ message: "phone ochiq" });
    ...
    async edit(req, res) {
        try {
            const { id, firstName, lastName, middleName, secondPhone, bio, adress, emails, phones, salary, } = req.body;
            if (!id)
                return res.status(400).json({ message: "id talab qilinadi" });
