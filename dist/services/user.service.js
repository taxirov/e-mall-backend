...
class UserService {
    async create(nickname, phone, password) {
        return await database_1.default.user.create({ data: { nickname, phone, password } });
    }
    async getById(id) {
    ...
    async clearToken(id) {
        return await database_1.default.user.update({ where: { id }, data: { activeToken: null, ipAddress: null, deviceName: null, isLoggedIn: false } });
    }
    async findAll() {
    ...
    async findByPhone(phone) {
        return await database_1.default.user.findUnique({ where: { phone } });
    }
    async findByNickname(nickname) {
        return await database_1.default.user.findUnique({ where: { nickname } });
    }
    async updatePassword(id, password) {
        return await database_1.default.user.update({ where: { id }, data: { password } });
    }
    async edit(id, data) {
        return await database_1.default.user.update({ where: { id }, data });
    }
    ...
    async createTelegramUser(phone, code, name, username, chatId) {
        return await database_1.default.userTelegram.create({ data: { phone, code, chatId, name, username } });
    }
    async getTelegramUser(code) {
        return await database_1.default.userTelegram.findMany({ where: { code, status: true } });
    }
    async deactiveTelegramUser(id) {
        return await database_1.default.userTelegram.update({ data: { status: false }, where: { id, status: true } });
    }
}
